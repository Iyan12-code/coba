...
function getTransformValues(el) {
  ...
  const values = transform.split(/[(,)]/).slice(1, -1).map((v) => parseFloat(v));
  if (values.length > 0 && !values.some(isNaN)) {
    transformCache.set(el, values);
  }
  return values;
}
function invalidateTransformCache(elements) {
  if (elements) {
...
var Carousel = defineComponent({
  ...
  setup(props, { slots, emit, expose }) {
    ...
    const setAnimationInterval = (event) => {
      ...
      invalidateTransformCache(/* @__PURE__ */ new Set([target]));
    ...
    const visibleRange = computed(() => {
      ...
      let minIndex = 0;
      {
        ...
        while (accumulatedSize <= offset && iterations < maxIterations) {
          ...
          const slideSize2 = ((_a2 = slidesRect.value[normalizedIndex]) === null || _a2 === void 0 ? void 0 : _a2[dimension.value]) || 0;
          if (slideSize2 <= 0)
            break;
          accumulatedSize += slideSize2 + config.gap;
          index++;
          iterations++;
        }
        minIndex = index - 1;
      ...
      let maxIndex = 0;
      {
        ...
        while (accumulatedSize < viewportRect.value[dimension.value] && iterations < maxIterations) {
          const normalizedIndex = (index % slides.length + slides.length) % slides.length;
          const slideSize2 = ((_b = slidesRect.value[normalizedIndex]) === null || _b === void 0 ? void 0 : _b[dimension.value]) || 0;
          if (slideSize2 <= 0)
            break;
          accumulatedSize += slideSize2 + config.gap;
          index++;
  ...
  setup(props, { attrs, slots, expose }) {
    ...
    return () => {
      ...
      return h("li", {
        ...
        tabindex: props.isClone || !isVisible.value ? -1 : void 0
